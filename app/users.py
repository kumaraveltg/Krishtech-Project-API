...
class Puser(BaseModel):
   ...
   # password: str = Field(nullable=False)
    #hpassword: str = Field(nullable=False)
    firstname: Optional[str] = None
    emailid: Optional[EmailStr] = None
...
@router.get("/userlist",response_model=List[Kusers])
def users_list(session: Session=Depends(get_session)):
    results = session.exec(select(Kusers))
    get_users_list = results.all()
    return get_users_list

@router.post("/update_user/{id}")
def update_user(id: int,update_user:Ukuser,session: Session=Depends(get_session)):
    db_users = session.get(Kusers,id)
    if not db_users:
